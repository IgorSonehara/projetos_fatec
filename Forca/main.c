...
void verificarSeArquivoExiste() // Se o arquivo n�o existir ele cria o arquivo sem apresentar mensagem de erro
{
    FILE *arq;
...
int verificarQtdPalavrasArquivo()
{
    ...
    else
    {
        ...
        if(ferror(arq) != 0)
        {
            printf("\nErro - N�o foi poss�vel ler o arquivo.");
        }
...
void incrementarQtdPalavrasArquivo()
{
    ...
    else
    {
        ...
        if(ferror(arq) != 0)
        {
            printf("\nErro - N�o foi poss�vel escrever a qtd no arquivo.");
        }
...
void escreverPalavraFimArquivo(char _palavra[TAM])
{
    ...
    else
    {
        ...
        if(ferror(arq) != 0)
        {
            printf("\nErro - N�o foi poss�vel escrever a palavra no arquivo.");
        }
...
void configurarJogo()
{
    ...
    while(opcao != 'B')
    {
        ...
        printf(">>> CONFIGURA��O DO JOGO:\n\n");

        printf("Escolha uma op��o:\n");
        printf("A) Adicionar nova palavra ao jogo.\n");
        printf("B) Concluir configura��o.\n\n");
        ...
        switch(opcao)
        {
            ...
            case 'B':
                break;
            default:
                printf("Op��o inv�lida! Tecle para continuar...");
                fflush(stdin);
        }
        scanf("%c", &c);
    }
...
int iniciarJogo()
{
    ...
    else
    {
		...
		if(!qtdPalavras) // Se n�o houver palavras no arquivo ele volta para o menu
        {
        ...
        if(qtdPalavras) // Caso haja ao menos uma palavra no arquivo, ele sorteia um n�mero aleat�rio entre 1 e o n�mero de palavras no arquivo
        {
        	char palavra[TAM];
...
void jogar(char palavra[TAM], char palavra_oculta[TAM])
{
    ...
    while(vida <= 6)
    {
        ...
        printf(" %s \n\n", palavra_oculta);
        printf(">>> Lista de Letras j� selecionadas: %s\n\n", letras_ja_selecionadas);

        if(vida != 6) // S� mostra o printf caso o jogador erre ao menos uma vez
        {
            printf(">>> Enforcado: \n\n");
        }
        ...
        char c;
        do // Faz com que o usu�rio digite apenas letras (adaptei da internet)
        {
            ...
            else if(c == 8) //se for pressionada a tecla BACKSPACE e houver caracteres j� digitados
            {
                letra[0] = '\0';
        ...
        }while(c != 13); // Quando o ENTER for pressionado o loop � encerrado
        ...
        for(i=0; i<strlen(palavra); i++) // Verifica se a letra ja foi digitada
        {
            if(letra[0] == palavra[i] || letra[0] == letras_ja_selecionadas[i])
            {
                ...
                {
                    printf("\nLetra inv�lida ou j� digitada, tente novamente...\n");
                    Sleep(1500);
                    x++;
                    break;
                }
                ...
                acertos++;
                x++; // Adiciona 1 ao contador x, dessa forma ele n�o entra no if abaixo
                }
            }
        }

        if(x == 0) // Caso a letra escolhida n�o seja correta
        {
        ...
        else if(acertos == strlen(palavra)-1) // Se o n�mero de acertos for igual ao tamanho da palavra o jogador ganha
            {
                printf("\n\n VOCE GANHOU!");
                printf("\n A PALAVRA �: %s", palavra);
                break;
